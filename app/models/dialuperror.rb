...
# !Внимание можно заменить строки
 self.table_name = "log_error_#{Bgmodule.find_by_name("dialup").id}" if !Rails.env.test?
 self.table_name = "#{ActiveRecord::Base.connection.execute("show tables like 'log_error_%';").first[0]}"  if Rails.env.test?
# на
